...
Node JS is one of the most powerful technologies to emerge within the last 15 years.
...
Doing `process.on('some-event', cb)` is the same as doing `process.addListener('some-event', cb)`.

Events Related to _process_:
...
In this guide, we're going to learn what it means to "open a web application".
...
JavaScript is gonna have to work in hand with C++, so that we could write JavaScript code to control C++ built-in features that allow us to control our computer's internal features. And these two together are known as NodeJS.
Why is it called Node JS when they're so much C++ in it still baffles me, but nevertheless - it is known as Node.

The model we are going to see over and over and over, again and again repeatedly, is:
...
We're going to get from JavaScript a TON of labels, built in to JavaScript that are gonna give us control. labels like "http", "fs", and not so many more, that are not built-in per-say, but you'd have to sort of summon on-demand (import).
...
We write JavaScript code, or more accurately - we use JavaScript label that Node had prepared for us, in order to trigger Node C++ code, that eventually reaches an Internal Feature.
...
It turns out, that http has a built-in method called createServer.
...
The "listen" function is a method on the returned JavaScript object which comes back from createServer.
...
As we've mentioned before, Node gives us an object full of edit functions, which we can then use to edit our outgoing response.
...
The first parameter passed to createServer, is actually saying "Hey Node, when the word/event `request` gets emitted, run doOnIncoming".
...
Basically, this was saying: "When a good message comes along, i.e. when the request event is emitted, invoke the function called doOnRequest". But what if a bad request comes along? For that we have an event name known as "clientError".
...
Alright, question! Do we have access to that file through JavaScript? Definitely not!
...
Here we're gonna do pretty much the same thing we did before, but this time we're gonna be a bit more sophisticated, and add a key pattern - a pattern known as the "Event System". The "Event System" is a system which behind the scenes, when things happen in the computer's internal features, Node is going to broadcast a message.
...
Deno mainly deviates from Node.js in the following aspects: 1. Supports only ES Modules like browsers where Node.js supports both ES Modules and CommonJS. CommonJS support in Deno is possible by using a compatibility layer.
...
Using exec we can run a command inside our shell, and get the output back.
...
- **stdout**: the response/output of a good execution of the command.
  ...
  If you try to run this, you'll get an error saying:
...
- env: Use env to specify environment variables that will be visible to the new process, the default is process.env. undefined values in env will be ignored.
  IMPORTANT TO KNOW!!!
